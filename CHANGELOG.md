...
Автостарт: файл flags/boot_renpy сразу открывает Ren'Py и не показывает BIOS. Кнопка BIOS из игры передаёт force_bios, чтобы из этого режима можно было выйти. На ПК тумблер ничего не пишет.

Обновление APK: если версия уже последняя, само не качается. Совпавший getFilesDir()/update.apk ставится без повторной загрузки. «Скачать заново» качает явно. После проверки в логе tag, своя версия и нужна ли качалка.
